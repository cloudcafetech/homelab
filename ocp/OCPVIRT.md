...
spec:
  type: LoadBalancer
  selector:
    kubevirt.io/domain: k8s-master # Matches your VM's autogenerated name label
  ports:
  - name: http
    protocol: TCP
...
echo "token: pkar-rke2" >> /etc/rancher/rke2/config.yaml
...
ln -s /var/lib/rancher/rke2/bin/kubectl /usr/local/bin/oc
mkdir -p ~/.kube
...
mkdir -p /etc/rancher/rke2 
echo "server: https://192.168.0.8:9345" > /etc/rancher/rke2/config.yaml		#Put Master IP
echo "token: pkar-rke2" >> /etc/rancher/rke2/config.yaml
